...
st.set_page_config(page_title="C7_日中デイトレ研究（寄り付き乖離・メカニズム検証版）", page_icon="🔁", layout="wide")
JST = ZoneInfo("Asia/Tokyo")

# =========================================================
# C7_app.py：C_app.py〜C6_app.pyと同じ手法（発見期間と検証期間の分離＋Bonferroni補正）で、
# 日中デイトレ戦略（寄り付き買い→大引け売り）そのものは変えずに、C6の「NAV乖離率」の切り口を
# さらに掘り下げる追加調査アプリ。C_app.py・C2_app.py・C3_app.py・C4_app.py・C5_app.py・
# C6_app.py・main_app.py・day_backtest.pyは変更していません。
#
# 背景：C6で使った「NAV乖離率」は前日終値時点の乖離だった（＝寄り付き買い→大引け売りという
# 固定の売買タイミングに対する判断材料の一つ）。一方で「乖離ができたときに買い、乖離が埋まった
# ときに儲かる」という発想をより直接的に検証するには、実際に自分が買う瞬間＝当日の寄り付き
# 時点で、前日確定のNAVに対してどれだけ乖離しているかを見る方が筋が良い。C7ではこれを
# 「当日寄り付き価格のNAV乖離率」として新たに追加する。
#
# このアプリは2つのことを検証する：
# ①儲けの検証：当日寄り付き時点の乖離を条件にすると、寄り付き買い→大引け売りの
#   リターン（day_ret）が平均より良くなるか。これはC1〜C6と同じBonferroni補正付きの
#   単一条件・2条件組み合わせ検証の枠組みにそのまま乗せる（44特徴量に拡張）。
# ②メカニズムの検証（参考情報・統計的検定はしない）：そもそも「乖離」自体は本当にその日の
#   うちに縮んでいるのか。寄り付き時点の乖離（前日NAV比）と、大引け時点の乖離（当日NAV比、
#   これは事後にしか分からない）を比べ、乖離の大きさが実際に縮小したかどうかを確認する。
#   「儲かったか」と「乖離が縮まったか」は別物（市場全体の値動きが混ざるため）なので、
#   ①とは別に、単純な記述統計として提示する。
# =========================================================
...
def build_research(start, end):
    ...
    # --- ここから「NAV乖離率」指標（C6で追加、C7で寄り付き時点版を追加） ---
    ...
    if not nav_df.empty:
        ...
        # ①儲けの検証で使う特徴量（すべて寄り付き前に確定している情報のみ）
        ...
        # ②メカニズム確認専用（同日の終値情報を含むため特徴量としては使わない。診断・記述統計のみ）
        jp["_diag_close_premium_pct"] = close_premium_pct
        jp["_diag_gap_narrowing"] = open_premium_pct.abs() - close_premium_pct.abs()
    else:
...
# UI
# =========================================================
st.title("🔁 C7_日中デイトレ研究（寄り付き乖離・メカニズム検証版）")
st.caption("C_app.py〜C6_app.pyと同じ発見期間・検証期間の分離＋多重検定補正の枠組みに、当日寄り付き時点のNAV乖離率を追加し、あわせて『乖離が実際に縮んでいるか』というメカニズムも別途確認します。")

st.info(
    "C_app.py・C2_app.py・C3_app.py・C4_app.py・C5_app.py・C6_app.py・main_app.py・day_backtest.pyは変更していません。これは追加調査専用の新アプリです。\n\n"
    "対象戦略：当日の1570を**寄り付きで買い、大引けで売る**（日中デイトレ）。デイトレの型はC6までと同じで変えていません。\n"
    "③④の統計的検定で使う判断材料は、寄り付き前に確定している情報のみです（当日の日経225・TOPIXの終値や、当日確定のNAVは使いません）。\n\n"
    "C6までのNAV乖離率は前日終値時点の乖離でした。C7ではこれに加えて、実際に買う瞬間である**当日寄り付き価格**が"
    "前日確定のNAVに対してどれだけ乖離しているか（open_nav_premium_pct）を新たな判断材料として追加しています。\n\n"
    "さらに②では、統計的検定とは別に「その乖離が実際に大引けまでに縮んでいるか」というメカニズムそのものを、"
    "参考情報として確認します（儲けの検証とメカニズムの検証は別物なので、分けて示します）。"
)
...
if run or "C7_df" not in st.session_state:
    with st.spinner("研究データを取得・整理しています（株価データ＋気象データ）…"):
        try:
            st.session_state["C7_df"] = build_research(all_start.isoformat(), all_end.isoformat())
        except Exception as e:
            st.error(f"データ取得エラー：{e}")
            st.stop()

df = st.session_state["C7_df"]
if df.empty:
...
st.write("毎日無条件で1570を寄り付きに買って大引けで売った場合を基準にします。NAV乖離率などの指標が、この基準よりも良い日だけを選び出せているかを検証します。")
...
# =========================================================
# ② メカニズムの検証（参考情報：統計的検定はしない）
# =========================================================
st.header("② メカニズムの検証（参考情報）")
st.caption(
    "ここは③④の統計的検定（Bonferroni補正付き）とは別の、単純な記述統計です。有意性検定はしていません。"
)
st.write(
    "day_ret（寄り付き買い→大引け売りの損益）にはその日の市場全体の値動きも混ざるため、"
    "『day_retが良い』ことと『乖離が縮んだ』ことは同じではありません。ここでは、寄り付き時点の乖離"
    "（open_nav_premium_pct）の大きさが、大引け時点の乖離（当日NAV比。事後にしか分からない）にかけて"
    "実際に縮んでいるかどうかを直接比較します。"
)

mech_cols = ["open_nav_premium_pct", "_diag_close_premium_pct", "_diag_gap_narrowing", TARGET]
...
if len(mech_df) < 50:
    ...
    for g, sub in mech_df.groupby("group", observed=True):
        rows.append({
            "グループ": g,
            "件数": len(sub),
            "平均day_ret(%)": sub[TARGET].mean(),
            "平均寄り付き乖離(%)": sub["open_nav_premium_pct"].mean(),
            "平均引け乖離(%)": sub["_diag_close_premium_pct"].mean(),
            "平均縮小幅(pt)": sub["_diag_gap_narrowing"].mean(),
            "縮小した日の割合(%)": (sub["_diag_gap_narrowing"] > 0).mean() * 100,
        })
    ...
    overall_narrow_pct = (mech_df["_diag_gap_narrowing"] > 0).mean() * 100
    overall_narrow_mean = mech_df["_diag_gap_narrowing"].mean()
    st.info(
        ...
        "「平均縮小幅」がプラスなら平均的に乖離は縮む方向、マイナスなら広がる方向だったことを意味します。"
        "縮小幅がグループ間で大きく変わらない、あるいは縮小した日の割合が50%前後で推移している場合、"
        "『乖離ができたから縮んで儲かる』というメカニズムを裏付ける強い証拠にはなりません。"
    )

st.caption(
    "※ この節で使う_diag_close_premium_pct・_diag_gap_narrowingは、当日の大引け時点の情報を含むため、"
    "③④の売買判断（寄り付き前に確定している情報のみ）には一切使っていません。あくまで事後のメカニズム確認用です。"
)

# =========================================================
# ③ 単一条件の発見・検証（NAV乖離率を含む44特徴量）
# =========================================================
st.header("③ 単一条件の発見・検証（NAV乖離率を含む44特徴量）")
st.caption("C6_app.pyの43指標に加え、当日寄り付き時点のNAV乖離率（open_nav_premium_pct）を1つ追加した44指標で検証します。")
...
st.header("⑤ 表の読み方")
st.markdown("""
**③④** はB02/B03/C_app.py〜C6_app.pyと同じく、多数の条件を同時に試すため
Bonferroni補正した厳しい基準で「🟢再現候補」を判定しています。

**②のメカニズム検証** は単純な記述統計であり、Bonferroni補正や有意性検定は行っていません（参考情報）。
