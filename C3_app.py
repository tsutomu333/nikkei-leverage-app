...
st.set_page_config(page_title="C3_日中デイトレ研究（高度指標版）", page_icon="🌅", layout="wide")
JST = ZoneInfo("Asia/Tokyo")

# =========================================================
# C3_app.py：C_app.py/C2_app.pyと同じ手法（発見期間と検証期間の分離＋Bonferroni補正）で、
# 「誰でも思いつく単純な水準・騰落率」ではなく、もう少し手の込んだ指標
# （値動きの大きさ・連続性・VIXの期間構造など）を追加して
# 日中デイトレ戦略（寄り付き買い→大引け売り）を検証する追加調査アプリ。
# C_app.py・C2_app.py・main_app.py・day_backtest.pyは変更していません。
#
# C2_app.pyとの違い：単一条件の候補を15種類→21種類に拡張。
# 追加した6指標：
#   - us_range_pct  ：前回米国NASDAQ100の値幅（高値-安値）そのもの＝市場の荒れ具合
#   - vix_term       ：VIX3M÷VIX（期間構造。1未満＝バックワーデーション＝市場ストレス）
#   - streak_us      ：米国NASDAQ100の連続陽線・陰線日数（符号付き）＝トレンドの続き具合
#   - realized_vol_1570：1570自身の直近10日ボラティリティ（当日を含まない過去のみ）
#   - prior_day_ret  ：1570自身の前日の日中リターン（寄り引け結果そのもの）
#   - vix_shock      ：VIXの変化率の絶対値（方向を問わない急変動）
...
def build_research(start, end):
    ...
    # 前回米国NASDAQ100の値幅（高値-安値）/終値：方向ではなく荒れ具合
    nq_df = raw["NASDAQ100"]
...
# UI
# =========================================================
st.title("🌅 C3_日中デイトレ研究（高度指標版）")
st.caption("C_app.py/C2_app.pyと同じ発見期間・検証期間の分離＋多重検定補正で、値動きの大きさ・連続性・VIXの期間構造など、より手の込んだ指標を追加して検証します。")

st.info(
    "C_app.py・C2_app.py・main_app.py・day_backtest.pyは変更していません。これは追加調査専用の新アプリです。\n\n"
...
if run or "C3_df" not in st.session_state:
    with st.spinner("研究データを取得・整理しています…"):
        try:
            st.session_state["C3_df"] = build_research(all_start.isoformat(), all_end.isoformat())
        except Exception as e:
            st.error(f"データ取得エラー：{e}")
            st.stop()

df = st.session_state["C3_df"]
if df.empty:
...
# =========================================================
# ② 単一条件の発見・検証（追加の高度指標を含む21特徴量）
# =========================================================
st.header("② 単一条件の発見・検証（高度指標を含む21特徴量）")
st.caption("C2_app.pyの15指標に加え、値幅・VIX期間構造・連続日数・1570自身のボラティリティと前日リターン・VIXショックの6指標を追加しています。")
...
st.header("④ 表の読み方")
st.markdown("""
**②③** はB02/B03/C_app.py/C2_app.pyと同じく、多数の条件を同時に試すため
