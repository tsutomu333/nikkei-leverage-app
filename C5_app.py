...
st.set_page_config(page_title="C5_日中デイトレ研究（複数日トレンド版）", page_icon="📈", layout="wide")
JST = ZoneInfo("Asia/Tokyo")

# =========================================================
# C5_app.py：C_app.py/C2_app.py/C3_app.py/C4_app.pyと同じ手法（発見期間と検証期間の分離＋
# Bonferroni補正）で、日中デイトレ戦略（寄り付き買い→大引け売り）そのものは変えずに、
# 判断材料にする指標だけを「前日1日だけの変化率」から「直近複数日の累積変化率」に広げて
# 検証する追加調査アプリ。C_app.py・C2_app.py・C3_app.py・C4_app.py・main_app.py・
# day_backtest.pyは変更していません。
#
# 背景：これまでのC〜C4はほとんどが「前日1日だけ」の変化率を指標にしていた。1日だけの
# 変化は偶然のノイズが混じりやすいが、複数日の累積なら多少ならされて、本当のトレンドを
# 拾える可能性がある。ただしデイトレという戦略の型（その日の寄り付き買い→大引け売り）は
# 変えず、あくまで「入力の指標」だけを広げる。
#
# C4_app.pyとの違い：単一条件の候補を26種類→40種類に拡張。
# 主要7指標（VIX・ドル円・NASDAQ100・NYダウ・原油・金・米国債ETF）について、
# 直近3日間・5日間の累積変化率をそれぞれ追加（7指標×2期間＝14指標追加）。
# 期間のバリエーションを増やしすぎるとBonferroni補正の基準が際限なく厳しくなるため、
# 3日・5日の2パターンに絞っている。
# =========================================================
...
# UI
# =========================================================
st.title("📈 C5_日中デイトレ研究（複数日トレンド版）")
st.caption("C_app.py〜C4_app.pyと同じ発見期間・検証期間の分離＋多重検定補正で、判断材料の指標を「前日1日」から「直近複数日の累積トレンド」に広げて検証します。")

st.info(
    "C_app.py・C2_app.py・C3_app.py・C4_app.py・main_app.py・day_backtest.pyは変更していません。これは追加調査専用の新アプリです。\n\n"
    "対象戦略：当日の1570を**寄り付きで買い、大引けで売る**（日中デイトレ）。デイトレの型はC4までと同じで変えていません。\n"
    "判断材料は寄り付き前に確定している情報のみ使用（当日の日経225・TOPIXの終値は使いません）。\n\n"
    "C4までの指標はほとんどが「前日1日だけ」の変化率でした。C5ではVIX・ドル円・NASDAQ100・NYダウ・原油・金・"
    "米国債ETFの7指標について、直近3日間・5日間の累積変化率を追加しています。"
)
...
if run or "C5_df" not in st.session_state:
    with st.spinner("研究データを取得・整理しています（株価データ＋気象データ）…"):
        try:
            st.session_state["C5_df"] = build_research(all_start.isoformat(), all_end.isoformat())
        except Exception as e:
            st.error(f"データ取得エラー：{e}")
            st.stop()

df = st.session_state["C5_df"]
if df.empty:
...
st.write("毎日無条件で1570を寄り付きに買って大引けで売った場合を基準にします。複数日の累積トレンド指標が、この基準よりも良い日だけを選び出せているかを検証します。")
...
# =========================================================
# ② 単一条件の発見・検証（複数日トレンドを含む40特徴量）
# =========================================================
st.header("② 単一条件の発見・検証（複数日トレンドを含む40特徴量）")
st.caption("C4_app.pyの26指標に加え、主要7指標の直近3日・5日累積変化率14指標を追加しています。")
...
st.header("④ 表の読み方")
st.markdown("""
**②③** はB02/B03/C_app.py〜C4_app.pyと同じく、多数の条件を同時に試すため
