...
st.set_page_config(page_title="C6_日中デイトレ研究（NAV乖離率版）", page_icon="⚖️", layout="wide")
JST = ZoneInfo("Asia/Tokyo")

# =========================================================
# C6_app.py：C_app.py〜C5_app.pyと同じ手法（発見期間と検証期間の分離＋Bonferroni補正）で、
# 日中デイトレ戦略（寄り付き買い→大引け売り）そのものは変えずに、1570自身の「構造」に
# 着目した新しい切り口＝NAV（基準価額）に対する市場価格の乖離率を追加指標として検証する
# 追加調査アプリ。C_app.py・C2_app.py・C3_app.py・C4_app.py・C5_app.py・main_app.py・
# day_backtest.pyは変更していません。
#
# 背景：C〜C5は市場データ（価格・水準・変化率）と天気という外部データを指標にしてきたが、
# 1570自身の「構造」は使っていなかった。ETFには取引所での市場価格と、ファンドの理論的な
# 価値である基準価額（NAV）の2つの値段があり、需給が偏ると市場価格がNAVに対してプレミアム
# （割高）・ディスカウント（割安）で取引されることがある。野村アセットマネジメント自身が
# 1570の乖離率を公式に公表しており、実在する現象であることは確認済み。この乖離が翌日以降に
# 解消される方向（平均回帰）に動くかを検証する。
#
# データソース：野村アセットマネジメントが公開する1570の基準価額履歴CSV
# （https://www.nomura-am.co.jp/fund/etf/history/ETF_1570.csv、Shift-JISエンコード）。
# 株価データ・天気データとは別の第3のソースで、1口当たり基準価額（NAV/口）の日次系列を含む。
#
# C5_app.pyとの違い：単一条件の候補を40種類→43種類に拡張。
# 「前日の市場終値がNAVに対してどれだけ乖離していたか」を軸に、乖離率の水準・直近3日間の
# 変化（拡大/縮小の勢い）・乖離の大きさ（方向を問わない絶対値）の3指標を追加。
# =========================================================
...
def build_research(start, end):
    ...
    # 別の第3のソース）と、既存のETF市場終値（yfinance）を同じ日付で突き合わせて乖離率を
    # 計算し、前日までに確定した値を寄り付き前の判断材料として使う。
    ...
    if not nav_df.empty:
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            premium_pct = (jp["etf_close"].values - nav_aligned.values) / nav_aligned.values * 100
        premium_pct = pd.Series(premium_pct, index=jp.index)
        jp["nav_premium_pct"] = premium_pct.shift(1)
        jp["nav_premium_chg_3d"] = premium_pct.shift(1) - premium_pct.shift(4)
...
# UI
# =========================================================
st.title("⚖️ C6_日中デイトレ研究（NAV乖離率版）")
st.caption("C_app.py〜C5_app.pyと同じ発見期間・検証期間の分離＋多重検定補正で、1570自身の「構造」＝市場価格のNAV乖離率を判断材料に追加して検証します。")

st.info(
    "C_app.py・C2_app.py・C3_app.py・C4_app.py・C5_app.py・main_app.py・day_backtest.pyは変更していません。これは追加調査専用の新アプリです。\n\n"
    "対象戦略：当日の1570を**寄り付きで買い、大引けで売る**（日中デイトレ）。デイトレの型はC5までと同じで変えていません。\n"
    "判断材料は寄り付き前に確定している情報のみ使用（当日の日経225・TOPIXの終値は使いません）。\n\n"
    "C5までの指標は市場データ・天気データが中心でした。C6では1570自身の構造に着目し、取引所での市場価格が"
    "ファンドの基準価額（NAV）に対してどれだけプレミアム/ディスカウントで取引されているかを、野村アセットマネジメント"
    "公開のNAV履歴データ（株価・天気とは別の第3のデータソース）から計算して追加しています。"
)
...
if run or "C6_df" not in st.session_state:
    with st.spinner("研究データを取得・整理しています（株価データ＋気象データ）…"):
        try:
            st.session_state["C6_df"] = build_research(all_start.isoformat(), all_end.isoformat())
        except Exception as e:
            st.error(f"データ取得エラー：{e}")
            st.stop()

df = st.session_state["C6_df"]
if df.empty:
...
st.subheader("⚖️ NAV乖離率データの取得状況（C6で追加）")
nav_cols = ["nav_premium_pct", "nav_premium_chg_3d", "nav_premium_abs"]
...
# =========================================================
# ② 単一条件の発見・検証（NAV乖離率を含む43特徴量）
# =========================================================
st.header("② 単一条件の発見・検証（NAV乖離率を含む43特徴量）")
st.caption("C5_app.pyの40指標に加え、1570市場価格のNAV乖離率関連3指標（水準・3日間の変化・絶対値）を追加しています。")
...
st.header("④ 表の読み方")
st.markdown("""
**②③** はB02/B03/C_app.py〜C5_app.pyと同じく、多数の条件を同時に試すため
