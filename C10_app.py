...
st.set_page_config(page_title="C10_日中デイトレ研究（乖離とday_retの回帰モデル検証版）", page_icon="📐", layout="wide")
JST = ZoneInfo("Asia/Tokyo")

# =========================================================
# C10_app.py：C9の事前登録仮説（寄り付き乖離を閾値で二分割する方式）では統計的に有意な
# 結果が得られなかったことを受け、二分割をやめて寄り付き乖離の大きさを連続量として扱う
# 回帰モデルで、day_retとの関係を直接推定する追加調査アプリ。
# C_app.py・C2_app.py・C3_app.py・C4_app.py・C5_app.py・C6_app.py・C7_app.py・C8_app.py・
# C9_app.py・main_app.py・day_backtest.pyは変更していません。
#
# 背景：C9では、C8が発見した閾値（open_nav_premium_abs ≥ 2.051、VIX ≥ 22.532）と乖離の符号を
# 組み合わせた4個の事前登録仮説（H1〜H4）を、寄り付き乖離を「閾値を超えたか否か」で二分割する
# 方式で検証したが、4件とも統計的に有意ではなかった（最大でも|t|=0.97）。この二分割方式は、
# 乖離が2.1%の日も8%の日も同じ「超えている」グループとして扱ってしまうため、乖離の大きさに
# 比例するはずの効果の強さという情報を捨てている可能性がある。
#
# C10では、寄り付き乖離の大きさ（符号つき、open_nav_premium_pct）を連続的な説明変数として、
# day_retとの関係を単回帰・重回帰で直接推定する（③、新設）。事前登録した回帰式は2つ：
# R1はday_ret ~ 寄り付き乖離（平均回帰仮説：傾きがマイナス）、R2はR1にVIX高ダミーとの交互作用項を
# 加えたもの（増幅仮説：交互作用の傾きがマイナス）。いずれもC8・C9で既に発見済みの閾値
# （2.051, 22.532）を交互作用ダミーの作成にのみ流用し、新たな変数探索は行わないため、
# ⑤⑥のような45特徴量の総当たりBonferroniスキャンよりも軽い補正（bonferroni_z(2)）で検定する。
# あわせて④でC9の事前登録仮説（二分割版）、⑤⑥でC_app.py〜C9_app.pyと同じ45特徴量×6分位点の
# 総当たりBonferroniスキャンをTARGET=day_retで従来どおり実行し、これまでの検証を再確認する。
# =========================================================
...
# UI
# =========================================================
st.title("📐 C10_日中デイトレ研究（乖離とday_retの回帰モデル検証版）")
st.caption("C9の事前登録仮説（閾値による二分割）では有意な結果が得られなかったことを受け、寄り付き乖離の大きさとday_retの関係を回帰モデルで直接推定し、関係の傾き（形）が統計的に有意かどうかを検証します。")

st.info(
    "C_app.py・C2_app.py・C3_app.py・C4_app.py・C5_app.py・C6_app.py・C7_app.py・C8_app.py・C9_app.py・main_app.py・day_backtest.pyは変更していません。これは追加調査専用の新アプリです。\n\n"
    "このアプリは1570を売買しません。目的変数は引き続き**day_ret＝寄り付き買い→大引け売りの損益（％）**です"
    "（C8のgap_narrowing_ptは②で参考表示のみ行います）。\n"
    "③④⑤⑥の統計的検定で使う判断材料（特徴量）は、すべて寄り付き前に確定している情報のみです（当日確定のNAVや大引け情報は使いません）。\n\n"
    "C7では『乖離（符号つき）を条件にday_retを予測できるか』を45特徴量×6分位点で総当たり検証し、統計的に有意な候補は0件でした。"
    "C8では『乖離の絶対値（大きさ）』が大引けまでの縮小幅そのものを予測できるかを検証し、有意な候補が見つかりましたが、"
    "これは方向を区別しない『縮みやすさ』の話でした。\n\n"
    "C9では、C8が見つけた閾値と乖離の符号を組み合わせ、『深い割安なら買いが有利、深い割高なら買いを避けるべき』という"
    "方向つきの仮説を、寄り付き乖離を**閾値で二分割**した事前登録仮説として直接day_retに対して検証しましたが（④に移動）、"
    "4件とも統計的に有意な結果は得られませんでした。\n\n"
    "C10では、この二分割をやめ、寄り付き乖離の大きさを**連続量**として扱う回帰モデルで、day_retとの関係の傾きを"
    "直接推定します（③、新設）。あわせて④でC9の事前登録仮説（二分割版）、⑤⑥で従来どおりの45特徴量総当たり"
    "Bonferroniスキャンも行います。\n\n"
    "⚠️ ここで統計的に有意な結果が出ても、それはバックテスト上の話であり、実運用での利益を保証するものではありません。"
)
...
if run or "C10_df" not in st.session_state:
    with st.spinner("研究データを取得・整理しています（株価データ＋気象データ）…"):
        try:
            st.session_state["C10_df"] = build_research(all_start.isoformat(), all_end.isoformat())
        except Exception as e:
            st.error(f"データ取得エラー：{e}")
            st.stop()

df = st.session_state["C10_df"]
if df.empty:
...
st.write("目的変数は『寄り付き買い→大引け売りの損益（day_ret、単位は％）』です。プラスならその日のデイトレが儲かったことを、マイナスなら損したことを意味します。この無条件平均を基準にして、③の方向つき仮説や④⑤の判断材料が、この基準よりも高いday_retの日だけを選び出せているかを検証します。")
...
with c1:
    ...
    a[1].metric("勝率（プラスだった日）", f"{base_tr['win']:.1f}%")
    a[2].metric("無条件の平均day_ret", f"{base_tr['mean']:+.3f}%")
with c2:
    st.subheader("✅ 検証期間")
    st.caption(f"{validate_start} ～ {validate_end}")
    a = st.columns(3)
    a[0].metric("件数", f"{base_va['n']:,}")
    a[1].metric("勝率（プラスだった日）", f"{base_va['win']:.1f}%")
    a[2].metric("無条件の平均day_ret", f"{base_va['mean']:+.3f}%")
...
# =========================================================
# ② 参考：寄り付き乖離の大きさ別に見たday_retと縮小幅（③の回帰モデル検証・④の事前登録仮説・⑤⑥の総当たり検定の前に、全体像を descriptive に確認）
# =========================================================
st.header("② 参考：寄り付き乖離の大きさ別に見たday_retと縮小幅")
st.caption(
    "ここはまだ③の回帰モデル検証や④の事前登録仮説検定、⑤⑥の統計的検定（Bonferroni補正付き）ではなく、単純な記述統計です。有意性検定はしていません。"
    "寄り付き時点の乖離の大きさ・符号でグループ分けし、大引けにかけてのday_retと乖離縮小幅の傾向を先に眺めておきます。"
)
st.write(
    "gap_narrowing_pt（乖離の縮小幅）はC8の目的変数で、方向を区別しない『縮みやすさ』の指標です。"
    "C10でも引き続き目的変数であるday_ret（実際の売買損益）と併記し、グループごとの傾向を比較できるようにしています。"
)
...
if len(mech_df) < 50:
    ...
    for g, sub in mech_df.groupby("group", observed=True):
        rows.append({
            "グループ": g,
            "件数": len(sub),
            "平均day_ret(%)【C10目的変数】": sub["day_ret"].mean(),
            "平均寄り付き乖離(%)": sub["open_nav_premium_pct"].mean(),
            "平均引け乖離(%)": sub["_diag_close_premium_pct"].mean(),
            "平均縮小幅(pt)【C8参考】": sub["gap_narrowing_pt"].mean(),
            "縮小した日の割合(%)【C8参考】": (sub["gap_narrowing_pt"] > 0).mean() * 100,
        })
    mech_table = pd.DataFrame(rows)
    st.dataframe(
        mech_table.style.format({
            "平均day_ret(%)【C10目的変数】": "{:+.3f}",
            "平均寄り付き乖離(%)": "{:+.3f}",
            "平均引け乖離(%)": "{:+.3f}",
            "平均縮小幅(pt)【C8参考】": "{:+.3f}",
            "縮小した日の割合(%)【C8参考】": "{:.1f}",
    ...
    st.info(
        ...
        f"{overall_narrow_pct:.1f}%の日で寄り付き時点より大引け時点の方が乖離が小さくなっていました（C8参考）。\n\n"
        "グループ間でday_retに差が付いていれば、寄り付き乖離の符号・大きさがday_ret（実際の売買損益）を"
        "予測する材料になっている可能性があります。この傾向が偶然でないかどうかを、③の回帰モデル検証、"
        "④の事前登録仮説、⑤⑥のBonferroni補正付き総当たり検定にかけます。"
    )
...
# =========================================================
# ③ 寄り付き乖離とday_retの回帰モデル検証（新設：連続量として直接推定）
# =========================================================
st.header("③ 寄り付き乖離とday_retの回帰モデル検証（新設：連続量として直接推定）")
st.markdown(
    "④で検証するC9の事前登録仮説（H1〜H4）は、寄り付き乖離を**閾値で二分割**し、"
    "『閾値を超えているかどうか』だけを見る方式でした。しかしこの方式では、乖離が2.1%の日も"
    "8%の日も同じ『超えている』グループとして扱われてしまい、乖離の**大きさに比例するはずの効果の強さ**"
    "という情報を捨てている可能性があります。\n\n"
    "ここでは寄り付き乖離（open_nav_premium_pct、符号つき）を**連続的な説明変数**のまま使い、"
    "day_retとの関係を回帰式で直接推定します。事前登録した回帰式は2つです。"
)
st.info(
    "**R1（平均回帰仮説）**：day_ret = a + b×寄り付き乖離。寄り付き時点で割高（プラス）なら大引けにかけて"
    "下がりやすく（day_retはマイナス）、割安（マイナス）なら上がりやすい（day_retはプラス）という平均回帰の"
    "考え方が正しければ、傾き b はマイナスになるはずです。\n\n"
    "**R2（増幅仮説）**：day_ret = a + b1×寄り付き乖離 + b2×(寄り付き乖離×VIX高ダミー)。"
    "VIX高ダミーはC8・C9で既に見つかったVIX ≥ 22.532を流用します。R1の平均回帰効果が、"
    "VIXが高い（不安心理が強い）日にさらに強まるなら、交互作用項の係数 b2 はマイナスになるはずです。\n\n"
    "いずれもC8・C9で既に発見済みの閾値をVIX高ダミーの作成にのみ流用し、新たな変数探索は行っていません。"
    "そのため補正はごく軽い bonferroni_z(2) にとどめています（⑤⑥の総当たりBonferroniスキャンとは切り離して検証します）。"
)

hyp_z_r = bonferroni_z(2) if use_correction else 1.0
...
def ols_fit(X, y):
    ...
    return {"beta": beta, "se": se, "t": tvals, "n": n, "r2": r2}

def eval_regression(train_df, valid_df, x_cols, coef_idx, min_n_r=50):
    tr_sub = train_df.dropna(subset=x_cols + ["day_ret"])
    va_sub = valid_df.dropna(subset=x_cols + ["day_ret"])
    if len(tr_sub) < min_n_r or len(va_sub) < min_n_r:
        return None
    Xtr = np.column_stack([np.ones(len(tr_sub))] + [tr_sub[c].values for c in x_cols])
    ytr = tr_sub["day_ret"].values.astype(float)
    Xva = np.column_stack([np.ones(len(va_sub))] + [va_sub[c].values for c in x_cols])
    yva = va_sub["day_ret"].values.astype(float)
    res_tr = ols_fit(Xtr, ytr)
    res_va = ols_fit(Xva, yva)
    b_tr = res_tr["beta"][coef_idx]
    b_va = res_va["beta"][coef_idx]
    t_va = res_va["t"][coef_idx]
    if not np.isnan(t_va) and t_va <= -hyp_z_r and b_va < 0 and b_tr < 0:
        verdict_r = "🟢 仮説通り・有意"
    elif b_va < 0:
        verdict_r = "🟡 仮説通りだが有意性不足"
    else:
        verdict_r = "🔴 仮説と逆方向"
    return res_tr, res_va, b_tr, b_va, t_va, verdict_r

train_reg = train.copy()
valid_reg = valid.copy()
train_reg["vix_high_dummy"] = (train_reg["vix_level"] >= VIX_THRESH).astype(float)
valid_reg["vix_high_dummy"] = (valid_reg["vix_level"] >= VIX_THRESH).astype(float)
train_reg["premium_x_vixhigh"] = train_reg["open_nav_premium_pct"] * train_reg["vix_high_dummy"]
valid_reg["premium_x_vixhigh"] = valid_reg["open_nav_premium_pct"] * valid_reg["vix_high_dummy"]

reg_rows = []

r1 = eval_regression(train_reg, valid_reg, ["open_nav_premium_pct"], coef_idx=1)
if r1 is not None:
    res_tr1, res_va1, b_tr1, b_va1, t_va1, v_r1 = r1
    reg_rows.append({
        "回帰式": "R1：day_ret = a + b×寄り付き乖離（平均回帰仮説：b<0）",
        "発見件数": res_tr1["n"], "発見b": b_tr1, "発見R²": res_tr1["r2"],
        "検証件数": res_va1["n"], "検証b": b_va1, "検証t値": t_va1, "検証R²": res_va1["r2"],
        "判定": v_r1,
    })

r2 = eval_regression(train_reg, valid_reg, ["open_nav_premium_pct", "premium_x_vixhigh"], coef_idx=2)
if r2 is not None:
    res_tr2, res_va2, b_tr2, b_va2, t_va2, v_r2 = r2
    reg_rows.append({
        "回帰式": "R2：day_ret = a + b1×寄り付き乖離 + b2×(寄り付き乖離×VIX高ダミー)（増幅仮説：b2<0）",
        "発見件数": res_tr2["n"], "発見b": b_tr2, "発見R²": res_tr2["r2"],
        "検証件数": res_va2["n"], "検証b": b_va2, "検証t値": t_va2, "検証R²": res_va2["r2"],
        "判定": v_r2,
    })

if not reg_rows:
    st.warning("回帰に必要なサンプル数（各期間50件以上）が確保できませんでした。")
else:
    reg_df = pd.DataFrame(reg_rows)
    st.caption(f"事前登録回帰式：2件。Bonferroni補正後の有意基準 |t| ≥ {hyp_z_r:.2f}（該当係数がマイナス側でこの基準を超えることを要求）")
    st.dataframe(
        reg_df.style.format({
            "発見b": "{:+.4f}", "発見R²": "{:.4f}", "検証b": "{:+.4f}", "検証t値": "{:+.2f}", "検証R²": "{:.4f}",
        }),
        use_container_width=True, hide_index=True
    )

    n_reg_green = int((reg_df["判定"] == "🟢 仮説通り・有意").sum())
    if n_reg_green > 0:
        st.success(f"⭐ {n_reg_green}件の回帰式が統計的に有意に支持されました。詳細は上の表を確認してください。")
    else:
        st.warning(
            "2件の事前登録回帰式のうち、統計的に有意に支持されたものはありませんでした。"
            "寄り付き乖離の大きさを連続量として扱っても、day_retとの間に統計的に有意な線形の関係は見つかりませんでした。"
        )

    st.caption(
        "※ この節の2回帰式は、C8・C9で既に発見済みの閾値（2.051、22.532）をVIX高ダミーの作成にのみ流用し、"
        "新たな変数探索は行っていません。そのため軽い補正（bonferroni_z(2)）で済ませていますが、⑤⑥のような"
        "45特徴量の総当たり探索と比べて『後追いで都合よく選んだ閾値ではないか』という懸念は小さいものの、"
        "ゼロではない点にはご留意ください。また、この回帰は**線形の関係のみ**を仮定しており、"
        "非線形の関係がある場合は検出できません。"
    )

# =========================================================
# ④ 方向つきルールの事前登録仮説検証（本題：儲けにつながるか）
# =========================================================
st.header("④ 方向つきルールの事前登録仮説検証（本題：儲けにつながるか）")
st.markdown(
    "C8では『寄り付き時点の乖離の大きさ（絶対値）』が大引けまでの縮小幅を予測できるかを検証し、"
    "**open_nav_premium_abs ≥ 2.051** という条件で統計的に有意な再現候補が見つかりました。"
    "しかしこれは乖離の**大きさ**の話であり、**方向**（割安か割高か）を区別していないため、"
    "そのままでは『買うべきか』の判断材料にはなりません。同じ縮小幅でも、割安で始まった日は"
    "価格がNAVに向かって**上がる**ことで縮み（買えば儲かる）、割高で始まった日は価格がNAVに向かって"
    "**下がる**ことで縮む（買うと損する）可能性があるためです。\\n\\n"
    "ここでは、C8が既に見つけたこの閾値（2.051）と、C8の2条件組み合わせで最も注目された "
    "**VIX ≥ 22.532** を、乖離の**符号**（割安／割高）と組み合わせた、あらかじめ絞り込んだ"
    "4個の仮説だけを、⑤⑥のような総当たりスキャンとは切り離して直接day_retに対して検証します。"
)
st.info(
    "**この節の統計的な考え方**：⑤⑥のBonferroni補正は『45特徴量×6分位点＝270通り（単一）／"
    "最大45通り（組み合わせ）を総当たりで探す』ことに対する補正でした。ここではそうした総当たり"
    "探索はせず、C8で**既に発見済みの閾値をday_retに対してそのまま当てはめるだけ**の、あらかじめ"
    "決めた4個の仮説だけを検定します。そのため補正はごく軽い bonferroni_z(4) にとどめています。"
)

hyp_z = bonferroni_z(4) if use_correction else 1.0

def eval_hypothesis(mask_train, mask_valid, base_tr_h, base_va_h, bullish, min_n_h=20):
    tr_h = stats(train.loc[mask_train, "day_ret"])
    va_h = stats(valid.loc[mask_valid, "day_ret"])
    if tr_h["n"] < min_n_h or va_h["n"] < min_n_h:
        return tr_h, va_h, np.nan, "⚪ サンプル不足"
    t_valid_h = welch_t(va_h["mean"], va_h["std"], va_h["n"], base_va_h["mean"], base_va_h["std"], base_va_h["n"])
    train_edge_h = tr_h["mean"] - base_tr_h["mean"]
    valid_edge_h = va_h["mean"] - base_va_h["mean"]
    if bullish:
        if not np.isnan(t_valid_h) and t_valid_h >= hyp_z and valid_edge_h > 0 and train_edge_h > 0:
            verdict_h = "🟢 仮説通り・有意"
        elif valid_edge_h > 0:
            verdict_h = "🟡 仮説通りだが有意性不足"
        else:
            verdict_h = "🔴 仮説と逆方向"
    else:
        if not np.isnan(t_valid_h) and t_valid_h <= -hyp_z and valid_edge_h < 0 and train_edge_h < 0:
            ...
            verdict_h = "🔴 仮説と逆方向"
    return tr_h, va_h, t_valid_h, verdict_h

hyp_rows = []

m_tr = candidate_mask(train, "open_nav_premium_pct", "low", -H_THRESH)
m_va = candidate_mask(valid, "open_nav_premium_pct", "low", -H_THRESH)
tr1, va1, t1, v1 = eval_hypothesis(m_tr, m_va, base_tr, base_va, bullish=True)
hyp_rows.append({"仮説": "H1：深いディスカウント（寄り付き乖離≤-2.051%）→ day_retはプラスに寄る",
                  "発見件数": tr1["n"], "発見平均day_ret": tr1["mean"],
                  "検証件数": va1["n"], "検証平均day_ret": va1["mean"], "検証t値": t1, "判定": v1})

m_tr = candidate_mask(train, "open_nav_premium_pct", "high", H_THRESH)
m_va = candidate_mask(valid, "open_nav_premium_pct", "high", H_THRESH)
tr2, va2, t2, v2 = eval_hypothesis(m_tr, m_va, base_tr, base_va, bullish=False)
hyp_rows.append({"仮説": "H2：深いプレミアム（寄り付き乖離≥+2.051%）→ day_retはマイナスに寄る",
                  "発見件数": tr2["n"], "発見平均day_ret": tr2["mean"],
                  "検証件数": va2["n"], "検証平均day_ret": va2["mean"], "検証t値": t2, "判定": v2})

m_tr = candidate_mask(train, "open_nav_premium_pct", "low", -H_THRESH) & candidate_mask(train, "vix_level", "high", VIX_THRESH)
m_va = candidate_mask(valid, "open_nav_premium_pct", "low", -H_THRESH) & candidate_mask(valid, "vix_level", "high", VIX_THRESH)
tr3, va3, t3, v3 = eval_hypothesis(m_tr, m_va, base_tr, base_va, bullish=True)
hyp_rows.append({"仮説": "H3：深いディスカウント かつ VIX≥22.532 → day_retはさらにプラスに寄る",
                  "発見件数": tr3["n"], "発見平均day_ret": tr3["mean"],
                  "検証件数": va3["n"], "検証平均day_ret": va3["mean"], "検証t値": t3, "判定": v3})

m_tr = candidate_mask(train, "open_nav_premium_pct", "high", H_THRESH) & candidate_mask(train, "vix_level", "high", VIX_THRESH)
m_va = candidate_mask(valid, "open_nav_premium_pct", "high", H_THRESH) & candidate_mask(valid, "vix_level", "high", VIX_THRESH)
tr4, va4, t4, v4 = eval_hypothesis(m_tr, m_va, base_tr, base_va, bullish=False)
hyp_rows.append({"仮説": "H4：深いプレミアム かつ VIX≥22.532 → day_retはさらにマイナスに寄る",
                  "発見件数": tr4["n"], "発見平均day_ret": tr4["mean"],
                  "検証件数": va4["n"], "検証平均day_ret": va4["mean"], "検証t値": t4, "判定": v4})

hyp_df = pd.DataFrame(hyp_rows)
st.caption(f"事前登録仮説：4件。Bonferroni補正後の有意基準 |z| ≥ {hyp_z:.2f}（H1・H3はt値が+側、H2・H4はt値が-側でこの基準を超えることを要求）")
st.dataframe(
    hyp_df.style.format({
        "発見平均day_ret": "{:+.3f}", "検証平均day_ret": "{:+.3f}", "検証t値": "{:+.2f}",
    }),
    use_container_width=True, hide_index=True
)

n_hyp_green = int((hyp_df["判定"] == "🟢 仮説通り・有意").sum())
if n_hyp_green > 0:
    st.success(f"⭐ {n_hyp_green}件の仮説が統計的に有意に支持されました。詳細は上の表を確認してください。")
else:
    st.warning(
        "4件の事前登録仮説のうち、統計的に有意に支持されたものはありませんでした。"
        "つまり、C8で見つかった『乖離の縮小しやすさ』は確認できても、それを方向つきの売買ルールに"
        "変換して統計的に有意な収益（day_ret）にはまだつながっていません。"
    )

st.caption(
    "※ この節の4仮説は、C8で目的変数（縮小幅）に対して既に発見済みの閾値（2.051, 22.532）を、"
    "一度もday_retに対してチューニングし直さずにそのまま転用しています。そのため軽い補正（bonferroni_z(4)）で"
    "済ませていますが、⑤⑥のような45特徴量の総当たり探索と比べて『後追いで都合よく選んだ閾値ではないか』"
    "という懸念は小さいものの、ゼロではない点にはご留意ください。"
)

# =========================================================
# ⑤ 単一条件の発見・検証（目的変数：day_ret、45特徴量）
# =========================================================
st.header("⑤ 単一条件の発見・検証（目的変数：day_ret、45特徴量）")
st.caption("C7_app.pyの44指標に加え、当日寄り付き時点のNAV乖離率の絶対値（open_nav_premium_abs）を1つ追加した45指標で、目的変数day_ret（寄り付き買い→大引け売りの損益）を45特徴量×6分位点の総当たりで検証します（③の回帰モデル検証・④の事前登録仮説とは別に、機械的な総当たり探索も一応行っておきます）。")
...
st.header("⑦ 表の読み方")
st.markdown("""
**⑤⑥** はB02/B03/C_app.py〜C9_app.pyと同じく、多数の条件を同時に試すため
Bonferroni補正した厳しい基準で「🟢再現候補」を判定しています。目的変数は
**day_ret（寄り付き買い→大引け売りの損益、単位は％）**です。

**②の参考テーブル** は単純な記述統計であり、Bonferroni補正や有意性検定は行っていません。

**③の回帰モデル検証** は寄り付き乖離を連続量のまま扱い、day_retとの関係を回帰式で直接推定した、
あらかじめ絞り込んだ少数の検証です（新設）。

**④の事前登録仮説** はC8で既に見つかった閾値を転用した、あらかじめ絞り込んだ少数の仮説検定であり、
⑤⑥の総当たりBonferroniスキャンとは補正の重さが異なります（⑤⑥よりゆるい代わりに、対象を絞っています）。

**「改善」** はその期間の無条件平均day_retと比べて、条件を付けることで平均day_retが何ポイント上がったかです。
""")

st.warning(
    "⚠️ 重要：この結果だけでmain_app.pyの判定ロジックを変更しません。"
    "『発見 → 未使用期間で検証 → 実運用でフォワード確認』の順で進めます。\n\n"
    "⚠️ さらに重要：③の回帰モデル検証、④の事前登録仮説、⑤⑥の総当たりスキャンのいずれで🟢が出たとしても、"
    "backtestは実際の約定条件（手数料・スリッページ・寄り付きの実際の約定価格）を完全には再現しません。"
    "統計的有意性は将来の利益を保証するものではありません。C7では44特徴量でday_retを予測する検証を行い、"
    "単一条件264通り中0件、2条件組み合わせ45通り中0件しか有意な結果が出ておらず、C9の④⑤・C10の⑤⑥はその再確認です。"
)
