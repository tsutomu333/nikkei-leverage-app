...
st.set_page_config(page_title="C9_日中デイトレ研究（方向つき乖離ルールの収益性検証版）", page_icon="🧭", layout="wide")
JST = ZoneInfo("Asia/Tokyo")

# =========================================================
# C9_app.py：C8で見つかった「寄り付き乖離の大きさ」の発見を、実際に儲けにつながる
# 「方向つきルール」に変換できるかを検証する追加調査アプリ。
# C_app.py・C2_app.py・C3_app.py・C4_app.py・C5_app.py・C6_app.py・C7_app.py・C8_app.py・
# main_app.py・day_backtest.pyは変更していません。
#
# 背景：C7では「寄り付き時点の乖離（符号つき、open_nav_premium_pct）」を条件にday_ret
# （寄り付き買い→大引け売りの損益）を予測できるかを45特徴量×6分位点で総当たり検証したが、
# 統計的に有意な候補は0件（単一264通り中0件、2条件組み合わせ45通り中0件）だった。
# C8では「乖離の絶対値（大きさ）」を新しい目的変数gap_narrowing_pt（縮小幅）に格上げして
# 検証し、有意な候補が見つかった（open_nav_premium_abs ≥ 2.051 など）。しかしこれは方向を
# 区別しない「縮みやすさ」の話であり、そのままでは「買うべきか」を教えてくれない。同じ縮小幅は、
# 割安で始まった日（価格が上がって縮む＝儲かる）にも割高で始まった日（価格が下がって縮む＝
# 買うと損する）にも起こりうるためである。
#
# C9では、C8が既に発見した閾値（open_nav_premium_abs ≥ 2.051、組み合わせ条件のVIX ≥ 22.532）
# を、乖離の符号（割安／割高）と組み合わせ、「深い割安なら買いが有利、深い割高なら買いを避ける
# べき」という方向つきの仮説を、あらかじめ絞り込んだ少数（4件）の事前登録仮説として直接day_ret
# に対して検証する（③）。この4仮説はC8で目的変数（縮小幅）に対して発見済みの閾値を、一度も
# day_retに対してチューニングし直さずにそのまま転用しているため、③④⑤のような45特徴量の
# 総当たりBonferroniスキャンよりも軽い補正（bonferroni_z(4)）で検定する。あわせて④⑤で、
# C_app.py〜C8_app.pyと同じ45特徴量×6分位点の総当たりBonferroniスキャンをTARGET=day_retで
# 従来どおり実行し、C7の検証を最新の特徴量セット（open_nav_premium_absを含む45特徴量）で
# 再確認する。
# =========================================================
...
# UI
# =========================================================
st.title("🧭 C9_日中デイトレ研究（方向つき乖離ルールの収益性検証版）")
st.caption("C8で見つかった『寄り付き乖離の大きさ』の発見を、乖離の符号（割安・割高）と組み合わせた方向つきルールに変換し、実際の売買損益（day_ret）に対して統計的に有意な効果があるかを、あらかじめ絞り込んだ少数の事前登録仮説として検証します。")

st.info(
    "C_app.py・C2_app.py・C3_app.py・C4_app.py・C5_app.py・C6_app.py・C7_app.py・C8_app.py・main_app.py・day_backtest.pyは変更していません。これは追加調査専用の新アプリです。\n\n"
    "このアプリは1570を売買しません。目的変数は再び**day_ret＝寄り付き買い→大引け売りの損益（％）**です"
    ...
    "方向つきの仮説を、あらかじめ絞り込んだ少数の事前登録仮説として直接day_retに対して検証します（③）。"
    "あわせて④⑤で従来どおりの45特徴量総当たりBonferroniスキャンも行います。\n\n"
...
if run or "C9_df" not in st.session_state:
    with st.spinner("研究データを取得・整理しています（株価データ＋気象データ）…"):
        try:
            st.session_state["C9_df"] = build_research(all_start.isoformat(), all_end.isoformat())
        except Exception as e:
            st.error(f"データ取得エラー：{e}")
            st.stop()

df = st.session_state["C9_df"]
if df.empty:
...
# =========================================================
# ② 参考：寄り付き乖離の大きさ別に見たday_retと縮小幅（③の事前登録仮説・④⑤の総当たり検定の前に、全体像を descriptive に確認）
# =========================================================
st.header("② 参考：寄り付き乖離の大きさ別に見たday_retと縮小幅")
st.caption(
    "ここはまだ③の事前登録仮説検定や④⑤の統計的検定（Bonferroni補正付き）ではなく、単純な記述統計です。有意性検定はしていません。"
    "寄り付き時点の乖離の大きさ・符号でグループ分けし、大引けにかけてのday_retと乖離縮小幅の傾向を先に眺めておきます。"
)
st.write(
    "gap_narrowing_pt（乖離の縮小幅）はC8の目的変数で、方向を区別しない『縮みやすさ』の指標です。"
    "C9の目的変数であるday_ret（実際の売買損益）と併記し、グループごとの傾向を比較できるようにしています。"
)
...
if len(mech_df) < 50:
    ...
    for g, sub in mech_df.groupby("group", observed=True):
        rows.append({
            "グループ": g,
            "件数": len(sub),
            "平均day_ret(%)【C9目的変数】": sub["day_ret"].mean(),
    ...
    st.info(
        ...
        "予測する材料になっている可能性があります。この傾向が偶然でないかどうかを、③の事前登録仮説と"
        "④⑤のBonferroni補正付き総当たり検定にかけます。"
    )
...
st.caption("C7_app.pyの44指標に加え、当日寄り付き時点のNAV乖離率の絶対値（open_nav_premium_abs）を1つ追加した45指標で、目的変数day_ret（寄り付き買い→大引け売りの損益）を45特徴量×6分位点の総当たりで検証します（③の事前登録仮説とは別に、機械的な総当たり探索も一応行っておきます）。")
...
st.header("⑥ 表の読み方")
st.markdown("""
**④⑤** はB02/B03/C_app.py〜C8_app.pyと同じく、多数の条件を同時に試すため
